...
MmWaveMiErrorModel::Mib (const SpectrumValue& sinr, const std::vector<int>& map, uint8_t mcs)
{
  ...
  //std::cout << "MmWaveMiErrorModel::Mib \n";
  
  double MI;
  double MIsum = 0.0;
  SpectrumValue sinrCopy = sinr;
  //std::cout << "sinrCopy =" << sinrCopy << "\n";
  //std::cout << "sinrCopy[1] =" << sinrCopy[1] << "\n";
  //std::cout << "map.at (0) =" << map.at (0)<< "\n";
  //std::cout << "map.at (71) =" << map.at (71)<< "\n";
  //std::cout << "map.size () =" << map.size () << "\n";
   
   for (uint32_t i = 0; i < map.size (); i++)
    {
	  double sinrLin = sinrCopy[map.at (i)]; //ESTÁ DANDO ERRO AQUI
...
MmWaveMiErrorModel::MappingMiBler (double mib, uint8_t ecrId, uint32_t cbSize)
{
  ...
  //std::cout << "MmWaveMiErrorModel::MappingMiBler \n";
  double b = 0;
  double c = 0;
...
MmWaveMiErrorModel::GetTbDecodificationStats (const SpectrumValue& sinr, const std::vector<int>& map, uint32_t size, uint8_t mcs, HarqProcessInfoList_t miHistory)
{
  ...
  //std::cout << "MmWaveMiErrorModel::GetTbDecodificationStats \n";
  double tbMi = Mib(sinr, map, mcs);
 // std::cout << "chamou o MIB \n";
  double MI = 0.0;
  double Reff = 0.0;
  NS_ASSERT (mcs < 29);
  if (miHistory.size ()>0)
...
double
MmWaveMiErrorModel::ErrorRateCtrl (double sinrAvg)
{
	double errorRate;
