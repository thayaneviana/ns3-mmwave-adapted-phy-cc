...
{
	//std::cout << "MmWaveUePhy::StartSlot \n";
	//unsigned slotInd = 0;
	...
	{
		//std::cout << "UE-PHY DL m_slotNum == 0 \n";
	...
	{
		//std::cout << "Ue-phy UL m_slotNum =! 0 \n";
		...
		SendCtrlChannels (pktBurst, ctrlMsg, slotPeriod-NanoSeconds(1.0), m_slotNum);
...
void
MmWaveUePhy::SendCtrlChannels (Ptr<PacketBurst> pb, std::list<Ptr<MmWaveControlMessage> > ctrlMsg, Time prd, uint8_t slotInd)
{
	//std::cout << "MmWaveUePhy::SendCtrlChannels \n";
	//Mudando para omnidireconal nas msgs de RACH
	Ptr<AntennaArrayModel> antennaArray = DynamicCast<AntennaArrayModel> (GetDlSpectrumPhy ()->GetRxAntenna());
	...
	m_downlinkSpectrumPhy->StartTxDlControlFrames(pb,ctrlMsg,prd,slotInd);
	NS_LOG_DEBUG("Entrei na mmwave-ue-phy e na função SendCtrlChannels");
}
...
MmWaveUePhy::GenerateDlCqiReport (const SpectrumValue& sinr)
{
	if(m_ulConfigured && (m_rnti > 0) && m_receptionEnabled)
	{
		if (Simulator::Now () > m_wbCqiLast + m_wbCqiPeriod)
		{
			...
			sinrdbmedia = sinrdbmedia/72;
			//std::cout<< "sinr (db) DOWNLINK= \t" << sinrdbmedia << "\n";
			//----------------------------
