...
MmWaveMacEnbMemberPhySapUser::ReceiveRachPreamble (uint32_t raId)
{
	...
	std::cout << "Recebi preâmbulo RACH \n";
}
...
MmWaveEnbMac::DoSubframeIndication (SfnSf sfnSf)
{
    ...
    // process received RACH preambles and notify the scheduler
		std::cout << "Process received RACH preambles and notify the scheduler \n";
...
MmWaveEnbMac::DoReceiveControlMessage  (Ptr<MmWaveControlMessage> msg)
{
	...
	//std::cout << "recebi msg de controle";

	switch (msg->GetMessageType())
	{
		case (MmWaveControlMessage::DL_CQI):
		{
			//std::cout<<"ENB recebe msg DL_CQI \n";
	  ...
	  {
			//std::cout<<"ENB recebe msg DL_HARQ \n";
