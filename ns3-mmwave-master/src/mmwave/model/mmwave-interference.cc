...
{ 
	...
	{
		//std::cout << "mmWaveInterference::StartRx, m_receiving == false \n";
		NS_LOG_LOGIC ("first signal");
		m_rxSignal = rxPsd->Copy ();
		//std::cout << "*m_rxSignal do StartRx = " << *m_rxSignal << "\n";
    ...
    {
		//std::cout << "mmWaveInterference::StartRx, m_receiving == true \n";
		NS_LOG_LOGIC ("additional signal" << *m_rxSignal);
     	...
     	//NS_ASSERT (Sum ((*rxPsd) * (*m_rxSignal)) == 0.0); //Comentei esse aqui
     	//std::cout << "mmWaveInterference::StartRx, entrou no ELSE \n";
    	//(*m_rxSignal) += (*rxPsd);
    }
}
...
mmWaveInterference::AddSignal (Ptr<const SpectrumValue> spd, const Time duration)
{
	...
	if (signalId == m_lastSignalIdBeforeReset)
    {
		...
		//std::cout << "mmWaveInterference::AddSignal";
		//std::cout << "m_lastSignalIdBeforeReset += 0x10000000";
    }
...
{ 
	...
	ConditionallyEvaluateChunk ();
	(*m_allSignals) += (*spd);
	//std::cout << "m_allsignals" << (*m_allSignals);
}
...
mmWaveInterference::ConditionallyEvaluateChunk ()
{
	...
	if (m_receiving && (Now () > m_lastChangeTime))
    {
	    ...
	    //std::cout << "*m_allSignals" << *m_allSignals << "\n";
		//std::cout << "*m_rxSignal" << *m_rxSignal << "\n";
		//std::cout << "interference" << interf << "\n";
		//std::cout << "sinr do interference" << sinr << "\n";
