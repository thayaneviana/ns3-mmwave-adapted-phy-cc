																...
																uint64_t imsi, SpectrumValue& sinr, SpectrumValue& power)
{
	...
	if ((slot_count/8+1)==1 && slot_count%8+1==1)
	{
	sprintf(fname2, "UE_%llu_UL_SINR_CTRL_dB.txt", (long long unsigned ) imsi);
	sinr_file = fopen(fname2, "a");
	}
	Values::iterator it = sinr.ValuesBegin();
	while(it!=sinr.ValuesEnd())
	{
		sinr_sum= sinr_sum + 10*log10(*it);
...
MmWavePhyRxTrace::ReportInterferenceTrace (uint64_t imsi, SpectrumValue& sinr)
{
	...
	char fname[255];
	char fname2[255];
	sprintf(fname, "UE_%llu_DL_SINR_dB.txt", (long long unsigned ) imsi);
	log_file = fopen(fname, "a");
	if (((slot_count/8+1)==28 && slot_count%8+1==4) || ((slot_count/8+1)==1 && slot_count%8+1==1))
	{
	sprintf(fname2, "UE_%llu_DL_SINR_CTRL_dB.txt", (long long unsigned ) imsi);
	sinr_file = fopen(fname2, "a");
	}
	Values::iterator it = sinr.ValuesBegin();
	while(it!=sinr.ValuesEnd())
...
void
MmWavePhyRxTrace::ReportPwTrace (Ptr<MmWavePhyRxTrace> phyStats, std::string path,
		uint64_t imsi, SpectrumValue& power)
{
	std::cout <<"ReportPwTrace"<< std::endl;
	phyStats->ReportPowerTrace (imsi, power);
...
{
    std::cout << "ReportPowerTrace"<< std::endl; 
...
MmWavePhyRxTrace::RxPacketTraceUeCallback (Ptr<MmWavePhyRxTrace> phyStats, std::string path, RxPacketTraceParams params)
{
	...
	/*m_rxPacketTraceFile << "DlRxTrace\t" << params.m_frameNum << "\t" << (unsigned)params.m_sfNum << "\t" << (unsigned)params.m_symStart
			...
			//m_rxPacketTraceFile << params.m_sinr << ",\n" << std::endl;*/
			
			m_rxPacketTraceFile << "DlRxTrace\t" << params.m_rnti << "\t" << params.m_frameNum << "\t" << params.m_sinr << std::endl;
			//m_rxPacketTraceFile << params.m_sinr << ",\n" << std::endl;
