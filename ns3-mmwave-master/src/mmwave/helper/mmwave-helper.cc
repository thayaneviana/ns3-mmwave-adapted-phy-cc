...
{
	//std::cout << "MmWaveHelper::InstallSingleUeDevice \n \n \n \n";
	...
	ulPhy->SetHarqPhyModule (harq);//Acrescentei essa linha
	...
	//std::cout << "MmWaveHelper::InstallSingleUeDevice --> (&MmWaveSpectrumPhy::UpdateSinrPerceived, dlPhy) \n";
	dlPhy->AddDataSinrChunkProcessor (pData);
	
	
	/*---------------- Acrescentei esse bloco ulPhy

	Ptr<mmWaveChunkProcessor> pCtrl = Create<mmWaveChunkProcessor> ();
	//pCtrl->AddCallback (MakeCallback (&MmWaveEnbPhy::GenerateDataCqiReport, phy));
	pCtrl->AddCallback (MakeCallback (&MmWaveSpectrumPhy::UpdateSinrPerceived, ulPhy));
	std::cout << "MmWaveHelper::InstallSingleUeDevice --> (&MmWaveSpectrumPhy::UpdateSinrPerceived, ulPhy) \n";
...
MmWaveHelper::InstallSingleEnbDevice (Ptr<Node> n)
{
	...
	if(!m_snrTest)
	{
		//std::cout << "MmWaveHelper::InstallSingleEnbDevice";
		pData->AddCallback (MakeCallback (&MmWaveEnbPhy::GenerateDataCqiReport, phy));
		pData->AddCallback (MakeCallback (&MmWaveSpectrumPhy::UpdateSinrPerceived, dlPhy));
		//pData->AddCallback (MakeCallback (&MmWaveSpectrumPhy::UpdateSinrPerceived, ulPhy));
		//std::cout << "MmWaveHelper::InstallSingleEnbDevice --> (&MmWaveSpectrumPhy::UpdateSinrPerceived, dlPhy) \n";
	}
	dlPhy->AddDataSinrChunkProcessor (pData);
...
MmWaveHelper::EnableTraces (void)
{
	...
	EnablePdcpTraces ();
	EnablePowerTraces ();
	//std::cout << "EnableTraces"<< std::endl;
}
...
{
	Config::Connect ("/NodeList/*/DeviceList/*/MmWaveEnbPhy/UlSinrTrace",
			MakeBoundCallback (&MmWavePhyRxTrace::UlSinrTraceCallback, m_phyStats));
...
void
MmWaveHelper::EnablePowerTraces (void)
{
	NS_LOG_FUNCTION_NOARGS ();
	//std::cout << "EnablePowerTraces"<< std::endl; 
	Config::Connect ("/NodeList/*/DeviceList/*/MmWaveUePhy/ReportPwTrace",
				MakeBoundCallback (&MmWavePhyRxTrace::ReportPwTrace, m_phyStats));
				//não está conseguindo chamar a função &MmWavePhyRxTrace::ReportPwTrace
	//std::cout << "EnablePowerTraces"<< std::endl; 
...
//Acrescentei a função abaixo
void
MmWaveHelper::CalculateSinrRach (NodeContainer c, NodeContainer d)
{
	for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
	{
		Ptr<Node> node = *i;
		
		for (NodeContainer::Iterator j = d.Begin (); j != d.End (); ++j)
		{
			Ptr<Node> enb = *j;
		CalculateSinrRachNode (node, enb);
		}
	}
}

void
MmWaveHelper::CalculateSinrRachNode (Ptr<Node> n, Ptr<Node> m)
{
	Ptr<MmWaveSpectrumPhy> dlPhy = CreateObject<MmWaveSpectrumPhy> ();
	Ptr<MmWaveSpectrumPhy> ulPhy = CreateObject<MmWaveSpectrumPhy> ();
	...
	Ptr<mmWaveChunkProcessor> pData = Create<mmWaveChunkProcessor> ();
		...
		//Chama função para atualizar a SINR recebida de cada nó para msg RACH
		pData->AddCallback (MakeCallback (&MmWaveSpectrumPhy::UpdateSinrPerceived, dlPhy));
		//std::cout << "Entra no CalculateSinrRachNode  \n";
}
