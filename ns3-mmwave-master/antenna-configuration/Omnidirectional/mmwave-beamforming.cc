	...
	while (vit != tempPsd->ValuesEnd ())
	{
		...
		if ((*vit) != 0.00)
		{
			...
			for (unsigned int pathIndex = 0; pathIndex < m_pathNum; pathIndex++)
			{
				...
				//std::cout<<"Smallscale= \n"<< smallScaleFading << std::endl;
			}
			*vit = (*vit)*(norm (subsbandGain));
			 //std::cout<<"Ganho= \n"<< norm (subsbandGain)<< std::endl;
...
if (enbAntennaArray->IsOmniTx ())
	{
		...
		//std::cout<<"Gain Omni = \t" << Sum (bfGain)/nbands << "\n";
		if (downlink)
		{
		NS_LOG_DEBUG ("DL Omni \t  RNTI: \t" << uePhy->GetRnti() << "\t Gain \t" << Sum (bfGain)/nbands << "\t RX PSD \t" << Sum(*bfPsd)/nbands); // print avg bf gain
		}
		else
		{
			//std::cout<< "UPLINK";
		NS_LOG_DEBUG ("UL Omni \t  RNTI: \t" << uePhy->GetRnti() << "\t Gain \t" << Sum (bfGain)/nbands << "\t RX PSD \t" << Sum(*bfPsd)/nbands);
		}
