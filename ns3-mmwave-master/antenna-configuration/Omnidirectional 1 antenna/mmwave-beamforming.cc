...
//Number of antennas
uint32_t txAntennaSizeAlamouti = 1; //1 antenna
uint32_t rxAntennaSizeAlamouti = 1; //1 antenna
			...
			for (unsigned rxAntennaIndex = 0; rxAntennaIndex < rxAntennaSizeAlamouti; rxAntennaIndex++)
				{
					...
					subsbandGain = subsbandGain + hsum; //soma dos quadrados de cada elemento da matriz H, norma de Frobenius.
						//std::cout<<"Alamouti";
					}
				}
			*vit = (*vit)*(norm (subsbandGain));
			//*vit = (*vit)*(hnorm);//rxAntennaSizeAlamouti);
