...
{
	//std::cout<< "MmWaveEnbPhy::StartSubFrame \n";
	NS_LOG_FUNCTION (this);
...
MmWaveEnbPhy::StartSlot (void)
{
	...
	//std::cout << "MmWaveEnbPhy::StartSlot \n";
  ...
  std::list <Ptr<MmWaveControlMessage > > dciMsgList;
	...
	{
		//std::cout << "ENB-PHY DL m_slotNum == 0 \n";
		...
		//Acrescentei essa chamada da função AddExpectedTb 
		//**********
		...
		SendCtrlChannels(pktBurst, ctrlMsgs, slotPeriod-NanoSeconds(1.0), currSlot); // -1 ns ensures control ends before data period
	}
	else if (m_slotNum == m_currSfNumSlots-1) // UL control slot
	{
		//std::cout << "ENB-PHY UL m_slotNum =! 0 \n";
...
void
MmWaveEnbPhy::SendCtrlChannels(Ptr<PacketBurst> pb, std::list<Ptr<MmWaveControlMessage> > ctrlMsgs, Time slotPrd, SlotAllocInfo& slotInfo)
{
	//std::cout << "MmWaveEnbPhy::SendCtrlChannels \n";
	/* Send Ctrl messages*/
	NS_LOG_FUNCTION (this<<"Send Ctrl");
	
		//Mudando para omnidireconal nas msgs de RACH e RAR
	/*Ptr<AntennaArrayModel> antennaArray = DynamicCast<AntennaArrayModel> (GetDlSpectrumPhy ()->GetRxAntenna());
		antennaArray->ChangeToOmniTx ();
	...
	//	msg->GetMessageType() == MmWaveControlMessage::RACH_PREAMBLE)
		//{
		   // std::cout<<"MUDOU PARA OMNIDIRECIONAL";
			antennaArray->ChangeToOmniTx ();
		//}
	}*/
	
	m_downlinkSpectrumPhy->StartTxDlControlFrames (pb, ctrlMsgs, slotPrd, slotInfo.m_slotIdx);
...
{ 
  ...
  //std::cout<< "GenerateDataCqiReport \t";
...
//  	int16_t sinrFp = LteFfConverter::double2fpS11dot3 (sinrdb);
  	...
  	sinrdbmedia = sinrdbmedia + sinrdb;
  	i++;
  	
  }
 //std::cout<< "i= \t" << i << "\n";
 //sinrdbmedia =  10 * std::log10 (sinrdbmedia/72);
 //std::cout<< "enb-phy - sinr (db) UPLINK= \t" << sinrdbmedia << "\n";
...
{
	//std::cout << "ENTROU MmWaveEnbPhy::PhyCtrlMessagesReceived \n";
	std::list<Ptr<MmWaveControlMessage> >::iterator ctrlIt = msgList.begin ();
   

	while (ctrlIt != msgList.end ())
	{
		//std::cout << "ENTROU NO WHILE MmWaveEnbPhy::PhyCtrlMessagesReceived \n";
		...
		else if (msg->GetMessageType() == MmWaveControlMessage::RACH_PREAMBLE)
	  {
			...
			std::cout << "received RACH_PREAMBLE with RapID \t" << rachPreamble->GetRapId () << "\n";
		}
