...
//Quantidade de antenas utilizadas pelo Alamouti
uint32_t txAntennaSizeAlamouti = 2; //2 antenas
uint32_t rxAntennaSizeAlamouti = 32; //2 antenas
...
complexVector_t*
MmWaveBeamforming::GetLongTermFadingOmni (Ptr<BeamformingParams> bfParams) const
{
	complexVector_t* longTerm = new complexVector_t();
	for (unsigned pathIndex = 0; pathIndex < m_pathNum; pathIndex++)
	{
...
{
	rxAntennaSizeAlamouti = 32;
	//std::cout << "GetChannelGainVector \n";
	NS_LOG_FUNCTION (this);
	...
	while (vit != tempPsd->ValuesEnd ())
	{
		...
		if ((*vit) != 0.00)
		{
			...
			*vit = (*vit)*(norm (subsbandGain));
			//std::cout<<"Beamforming= \n"<< std::endl;
			//std::cout<<"Ganho beamforming="<< norm (subsbandGain) << "\n" << std::endl;
		}
		vit++;
		iSubband++;
	}
...
Ptr<SpectrumValue>
MmWaveBeamforming::GetChannelGainVectorOmni (Ptr<const SpectrumValue> txPsd, Ptr<BeamformingParams> bfParams, double speed) const
{
	rxAntennaSizeAlamouti = 32; //antenas no receptor eNB (uplink)
	//std::cout << "GetChannelGainVectorOmni \n";
	NS_LOG_FUNCTION (this);
	Ptr<SpectrumValue> tempPsd = Copy<SpectrumValue> (txPsd);
	if(m_fixSpeed)
	{
	...
	while (vit != tempPsd->ValuesEnd ())
	{
		...
		{
			double fsb = m_phyMacConfig->GetCentreFrequency () - GetSystemBandwidth ()/2 + m_phyMacConfig->GetChunkWidth ()*iSubband ;
			for (unsigned rxAntennaIndex = 0; rxAntennaIndex < rxAntennaSizeAlamouti; rxAntennaIndex++)
				{
					for (unsigned txAntennaIndex = 0; txAntennaIndex < txAntennaSizeAlamouti; txAntennaIndex++)
					{
						std::complex<double> hsum (0,0);
						for (unsigned int pathIndex = 0; pathIndex < m_pathNum; pathIndex++)
						{
							double sigma = bfParams->m_channelMatrix.m_powerFraction.at (pathIndex);
							double temp_delay = -2*M_PI*fsb*DelaySpread[pathIndex];
							...
							//somatório para o número de percursos
							//hsum = hsum + bfParams->m_channelMatrix.m_ueSpatialMatrix.at (pathIndex).at (rxAntennaIndex)*
							//bfParams->m_channelMatrix.m_enbSpatialMatrix.at (pathIndex).at (txAntennaIndex)*smallScaleFading;
							//std::cout << "rxAntennaSizeAlamouti=" << rxAntennaSizeAlamouti << "\n";
						     hsum = hsum + bfParams->m_channelMatrix.m_ueSpatialMatrix.at (pathIndex).at (txAntennaIndex)*
							 bfParams->m_channelMatrix.m_enbSpatialMatrix.at (pathIndex).at (rxAntennaIndex)*smallScaleFading;
						}
					hnorm = hnorm + norm(hsum); //soma dos quadrados de cada elemento da matriz H, norma de Frobenius.
						//std::cout<<"Alamouti";
					}
				}
			//*vit = (*vit)*(norm (subsbandGain));
			*vit = (*vit)*(hnorm)/txAntennaSizeAlamouti;
			//std::cout<<"Ganho alamouti="<< hnorm <<"\n"<< std::endl;
		}
		vit++;
		iSubband++;
	}
...
Ptr<SpectrumValue>
MmWaveBeamforming::GetChannelGainVectorOmniDownlink (Ptr<const SpectrumValue> txPsd, Ptr<BeamformingParams> bfParams, double speed) const
{
	//std::cout << "GetChannelGainVectorOmniDownlink \n";
	rxAntennaSizeAlamouti = 16; //antenas no receptor UE (downlink)
	NS_LOG_FUNCTION (this);
	Ptr<SpectrumValue> tempPsd = Copy<SpectrumValue> (txPsd);
	if(m_fixSpeed)
	{
												...
												Ptr<const MobilityModel> b) const
{
	...
	if (enbAntennaArray->IsOmniTx ())
	{
		...
		(*bfParams->m_beam) = vec;
		//bfParams->m_beam = GetLongTermFadingOmni (bfParams);// Ganho ALAMOUTI
	}
	else
	{
	...
	//se for Omnidirecional, calcula o bfPsd com outra função
	if (enbAntennaArray->IsOmniTx ())
	{
		//std::cout<<"Omni Alamouti \n";
		
		if (downlink)
		{
		//std::cout << "DOWNLINK \n";
		Ptr<SpectrumValue> bfPsd = GetChannelGainVectorOmniDownlink (rxPsd, bfParams,  relativeSpeed);
		//std::cout << "rxAntennaSizeAlamouti=" << rxAntennaSizeAlamouti << "\n";
		return bfPsd;
		}
		else
		...
		Ptr<SpectrumValue> bfPsd = GetChannelGainVectorOmni (rxPsd, bfParams,  relativeSpeed);
		//std::cout << "rxAntennaSizeAlamouti=" << rxAntennaSizeAlamouti << "\n";
		return bfPsd;
		}
		//std::cout << "GetChannelGainVectorOmni";
		...
		//std::cout<<"\t Gain Alamouti = \t" << Sum (bfGain)/nbands << "\n";
		/*if (downlink)
		{
		NS_LOG_DEBUG ("DL Alamouti \t  RNTI: \t" << uePhy->GetRnti() << "\t Gain \t" << Sum (bfGain)/nbands << "\t RX PSD \t" << Sum(*bfPsd)/nbands); // print avg bf gain
		}
		else
		{
			//std::cout<< "UPLINK";
		NS_LOG_DEBUG ("UL Alamouti \t  RNTI: \t" << uePhy->GetRnti() << "\t Gain \t" << Sum (bfGain)/nbands << "\t RX PSD \t" << Sum(*bfPsd)/nbands);
		}*/
	...
	else 
	{
		...
		SpectrumValue bfGain = (*bfPsd)/(*rxPsd);
		int nbands = bfGain.GetSpectrumModel ()->GetNumBands ();
		//std::cout<<"\t Gain Beamforming= \t" << Sum (bfGain)/nbands << "\n";
		if (downlink)
		{
			NS_LOG_DEBUG ("DL BF \t  RNTI: \t" << uePhy->GetRnti() << "\t Gain \t" << Sum (bfGain)/nbands << "\t RX PSD \t" << Sum(*bfPsd)/nbands); // print avg bf gain
		}
		else
		{
			//std::cout<< "UPLINK";
			NS_LOG_DEBUG ("UL BF \t  RNTI: \t" << uePhy->GetRnti() << "\t Gain \t" << Sum (bfGain)/nbands << "\t RX PSD \t" << Sum(*bfPsd)/nbands);
		}
