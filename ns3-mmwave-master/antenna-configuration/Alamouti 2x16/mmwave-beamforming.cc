...
uint32_t txAntennaSizeAlamouti = 2; //2 antenans
uint32_t rxAntennaSizeAlamouti = 16; //16 antennas
...
MmWaveBeamforming::GetLongTermFadingOmni (Ptr<BeamformingParams> bfParams) const
{
	...
	for (unsigned pathIndex = 0; pathIndex < m_pathNum; pathIndex++)
	{
		...
		NS_LOG_INFO ("rxsum="<<rxsum.real ()<<" "<<rxsum.imag ());
		longTerm->push_back (norm(txsum*rxsum));
	}
	return longTerm;
}
	...
	while (vit != tempPsd->ValuesEnd ())
	{
		...
		if ((*vit) != 0.00)
		{
			...
			for (unsigned rxAntennaIndex = 0; rxAntennaIndex < rxAntennaSizeAlamouti; rxAntennaIndex++)
				{
					...
					//std::cout<< "norm(hsum)=" << norm(hsum) << std::endl;
			...
			*vit = (*vit)*(hnorm/txAntennaSizeAlamouti);
			//std::cout<< "hnorm final=" << hnorm << std::endl;
			//std::cout<< "txAntennaSizeAlamouti" << txAntennaSizeAlamouti << "\n";
	...
	while (vit != tempPsd->ValuesEnd ())
	{
		...
		if ((*vit) != 0.00)
		{
			...
			for (unsigned int pathIndex = 0; pathIndex < m_pathNum; pathIndex++)
			{
				...
				hnorm = hnorm + (*bfParams->m_beam).at (pathIndex)*norm(smallScaleFading);
			}
			//hnorm = norm(hsum);
			*vit = (*vit)*(hnorm/txAntennaSizeAlamouti);
			//std::cout<<"Beamforming= \n"<< std::endl;
